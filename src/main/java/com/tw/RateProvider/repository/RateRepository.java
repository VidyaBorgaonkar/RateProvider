package com.tw.RateProvider.repository;
...
public class RateRepository {
    ...
    public List<Rate> getRatesByHotel(String hotelCode, String provider){
        if(!rateMap.keySet().contains(hotelCode)) {
            throw new IllegalArgumentException(hotelCode);
        }


        return rateMap.get(hotelCode).stream().filter(rate -> rate.getProvider() == provider).collect(Collectors.toList());
    }
